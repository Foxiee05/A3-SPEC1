...
let instructionImg;
let showInstructions = true; //instruction shows first when the canvas loads
...
// ***********************************************
function preload() {
  for (let i = 0; i < 10; i++) { //intro name
    introName[i] = loadImage(`assets/going-home-${i+1}.png`);
  }
  ...
  backBtnImg = loadImage('assets/back-button.png');
  ...
  //BAD PHASE OVERLAY------------------------------------------------
  for (let i = 1; i <= 5; i++) {
  badPhaseImages.push(loadImage(`assets/badPhase-overlay-${i}.png`));
}
...
function setup() {
  ...
  //NEW ROAD----------------------------------------------------------------
  roadLayer = new RoadScroll(roadImg, roadSpeed, 605 - 100);
  ...
  midTreeLayer = new ScrollingLayer(midTreeImages, midTreeSpeed, 615 - 100, 1000, 600, false);

  houseLayer = new ScrollingLayer(houseImages, roadSpeed, 615 - 100, 800, houseSpacing, true);
  
  frontTreeLayer = new ScrollingLayer(frontTreeImages, roadSpeed, 820 - 100, 1000, 500, false);

  

  //INITIAL SPAWNING FOR VEHICLES----------------------------------------------------------------
  bikerLayer = new VehicleLayer(bikerImages, bikerSpeed, 676 - 100, 4000);

  bikerSecondLayer = new VehicleSecondLayer(bikerImages, bikerSpeed + 10, 783 - 100, 4000);

  bigVehicleLayer = new VehicleLayer(bigVehicleImages, bigVehicleSpeed, 676 - 100, 8000);

  bigVehicleSecondLayer = new VehicleSecondLayer(bigVehicleImages, bigVehicleSpeed + 10, 783 - 100, 8000);



  //NEW FRAMES----------------------------------------------------------------
  /*using the same class with the road*/
  framesLayer = new RoadScroll(frameImg, frameSpeed, 748 - 100);


  //INITIAL SPAWNING FOR BRICKS----------------------------------------------------------------
  //constructor(imgs, speed, minY, maxY, minSpawnRate, maxSpawnRate, initialMin, initialMax)
  brickDitchLayer = new stackingDitch(brickImages, roadSpeed, 1000 - 100, 1010 - 100, 1000, 1100, 200, 400);


  //INITIAL SPAWNING FOR DIRT----------------------------------------------------------------
  dirtDitchLayer = new stackingDitch(dirtImages, roadSpeed, 895 - 100, 950 - 100, 700, 800, 100, 200);


  //INITIAL SPAWNING FOR SAND-BAGS----------------------------------------------------------------
  sandBagLayer = new stackingDitch([sandBagImages], roadSpeed, 850 - 100, 860 - 100, 100, 6000, 1000, 2000);

  
  //INITIAL SPAWNING FOR BUSHES----------------------------------------------------------------
  bushLayer = new stackingDitch(bushImages, roadSpeed, 900 - 100, 980 - 100, 500, 1500, 300, 700);


  //INITIAL SPAWNING FOR SIGNS----------------------------------------------------------------
  signLayer = new stackingDitch(signImages, roadSpeed, 830 - 100, 830 - 100, 10000, 10000, 1500, 1500);
  ...
  daBacteriaNetwork = new BacteriaNetwork(bacteriaImages, 80, 200, 976, 575 - 100, 150);
  pop();
...
function draw() {
  ...
  noStroke();
  rect(0, 937 - 100, 1920, 143 + 100); // Rect returns to black as clickCount hits 0
...
//CLOUDS SCROLLING + INTERACTION----------------------------------------------------------------
 ...
 //FISHER AREA-----------------------------------------------------------------
  ...
  //BAD PHASE OVERLAY----------------------------------------------------------------
  if (clickCount < maxClicks && daBacteriaNetwork.currentGrowthY <= 600) {
  //random image each 40 frames
  if (frameCount % 40 === 0) {
    currentBadImg = random(badPhaseImages);
    badX = random(0, 1400); 
    badY = random(0, 1080);
    
    //random rotation
    badRotation = random(0,360);
    
    //random flip (1 or -1)
    badScaleX = random(1) > 0.5 ? 1 : -1;
    badScaleY = random(1) > 0.5 ? 1 : -1;
  }

  let badShake = (variationIndex === 2) ? 2 : 0; //shake only in the last phase
 

  //draw the overlay if there is a current image
  if (currentBadImg) {
    push();
    //positioning + shake
    translate(badX + random(-badShake, badShake), badY + random(-badShake, badShake));
    
    //rotate + flip
    rotate(badRotation);
    scale(badScaleX, badScaleY);
    
    imageMode(CENTER);
    image(currentBadImg, 0, 0);
    pop();
  }
}
  ...
  if (clickCount < maxClicks) {
    ...
    if (daBacteriaNetwork.currentGrowthY <= daBacteriaNetwork.endY + 100) variationIndex = 2;
  }
  ...
  image(emotionImgs[variationIndex], 1586 + random(-s, s), 46 + random(-s, s));
  image(treeRootImgs[variationIndex], 1586 + random(-s, s), 318 + random(-s, s));
  image(legImgs[variationIndex], 1758 + random(-s, s), 317 + random(-s, s));
  image(grassImgs[variationIndex], 1592 + random(-s, s), 672 + random(-s, s));
  ...
  /*let instructBtn;
   ...
   //instructions button
   let btnX = 1594, btnY = 855, btnW = 293, btnH = 59;
   drawInteractiveButton(instructionBtnImg, btnX, btnY, btnW, btnH);

   //back button
   let backX = 1592, backY = 930, backW = 142, backH = 103;
   drawInteractiveButton(backBtnImg, backX, backY, backW, backH);

   //next button
  //  let nextX = 1748, nextY = 930, nextW = 142, nextH = 103;
  //  drawInteractiveButton(nextBtnImg, nextX, nextY, nextW, nextH);
