...
class SmallBubble {
  ...
  display() {
    //calculate distance from mouse to center of bubble for hover detection
    let d = dist(mouseX, mouseY, this.x + this.w / 2, this.y + this.h / 2);
    
    //check if mouse is in bubble boundaries
    if (d < this.w / 2) {
      //10% increment in size, offset to stay centered
      image(
        this.img, 
        this.x - (this.w * 0.05), 
        this.y - (this.h * 0.05), 
...
class BacteriaRawManager {
  constructor(imgs) {
    ...
    this.particles = [];
    this.areas = [
      { x: 1598, y: 408, w: 140, h: 232 },
      { x: 1768, y: 450, w: 119, h: 207 },
      { x: 1603, y: 691, w: 274, h: 122 }
    ];
  }
